...
def evaluate_nn_module(
    ...
    if (is_inductor_test or is_export_test) and run_dir:
        ...
        log_name = os.path.join(
            run_dir, f"inductor_logs/{repo_name}.{nn_cls.__name__}.txt"
        )
...
def evaluate_pyfile_subproc(tempdir: str, path: str, args, run_dir: str = None):
    ...
    nn_cls, get_forward_args, get_init_args = import_Model_and_args_from_code(
        code, module_name
    )
...
def evaluate_all(
    ...
    if args.synthetic_data_dir:
        synthetic_files = [
            f for f in os.listdir(args.synthetic_data_dir) if f.endswith(".py")
        ]
        ...
        if args.shard_num > len(synthetic_files):
            ...
            synthetic_files = [
                ...
                if i % args.shard_total == args.shard_num
            ]
        for f in synthetic_files:
            ...
            synthetic_file = os.path.join(
                synthetic_tests_dir, f"POPCORN_SYNTHETIC_DATA.{f}"
            )
