...
def extract_target_module(file_content: str, target_module: str) -> str:
    ...
    header_nodes = [
        node for node in header_nodes if isinstance(node, (ast.Import, ast.ImportFrom))
    ]
    ...
    if target_module not in class_dict:
        raise ValueError(
            f"Target module '{target_module}' not found among class definitions."
        )
    ...
    selected_order = [
        name
        for name, _ in sorted(order_list, key=lambda x: x[1])
        if name in selected_classes
    ]

    # --- Step 6. Get the selected functions
    selected_function_nodes = [
        f for f in function_nodes if f.name in selected_functions
    ]
        ...
        if isinstance(test_assign.value, ast.List):
            for elt in test_assign.value.elts:
                if isinstance(elt, ast.Tuple) and elt.elts:
                    first_elem = elt.elts[0]
                    if (
                        isinstance(first_elem, ast.Name)
                        and first_elem.id == target_module
                    ):
                        filtered_elts.append(elt)
            new_list_node = ast.List(elts=filtered_elts, ctx=ast.Load())
            new_test_assign = ast.Assign(
                targets=test_assign.targets, value=new_list_node
            )
