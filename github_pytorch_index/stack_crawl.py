...
ds = load_dataset('bigcode/the-stack-dedup', data_dir='data/python', streaming=True, split='train')
	...
	for torch_import_string in torch_import_strings:
		if torch_import_string in content:
			...
			if len(torch_repos) % 1000 == 0 and len(torch_repos) > 0:
				print(f'Current count of repositories with torch imports: {len(torch_repos)}')
			break

try:
	# sort the repo_dicts by stars - handle None values by treating them as -1
	sorted_repo_dicts = sorted(
		repo_dicts, key=lambda x: x['stars'] if x['stars'] is not None else -1, reverse=True
	)
