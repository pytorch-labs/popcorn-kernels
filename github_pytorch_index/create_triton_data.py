...
def strip_main_and_benchmarks(code):
	...
	for node in tree.body:
		...
		if isinstance(node, ast.FunctionDef) and node.name == 'benchmark_compiled_module':
			continue
...
def get_num_outputs_from_forward_call(pytorch_code, entry_point):
	...
	if not return_nodes:
		raise ValueError(f"No return statement found in the 'forward' method of '{entry_point}'.")
...
def remove_unused_module_level_constants(code):
	...
	unused_constants = {var: lines for var, lines in constants.items() if var not in used_vars}
	...
	filtered_lines = [line for i, line in enumerate(lines) if i not in line_indices_to_remove]
