		...
		if isinstance(node.value, ast.Attribute):
			...
			if getattr(node2.value, 'id', '') == 'torch' and node2.attr == 'cuda':
				if hasattr(torch, node.attr):
					...
					new_node = ast.Attribute(value=node2.value, attr=node.attr, ctx=node.ctx)
...
def split_import(node):
	...
	if isinstance(node, ast.Import):
		...
		for name in node.names:
			tmp = ast.ImportFrom(re.sub(r'^torch.legacy\b', 'torch', node.module), [name], level=0)
			ast.copy_location(tmp, node)
			yield module_name, tmp
...
class FlattenStatement(ast.NodeTransformer):
	...
	def visit_ListComp(self, node):
		return self._comprehension(node, 'append', ast.List([], ctx=ast.Load()), [node.elt])
