...
def generate_zipfile_subproc(tempdir: str, path: str, args):
	...
	test_path = '{}/test_{}.py'.format(test_dir, re.sub(r'([.]zip|/)$', '', os.path.basename(path)))
...
def generate_all(args, download_dir, limit=None, jobs=4, chunk_num=None, num_chunks=None):
	start = time.time()
	stats = Stats()
	errors = ErrorAggregatorDict()
	zipfiles = [
		os.path.join(download_dir, f) for f in os.listdir(download_dir) if f.endswith('.zip')
	]
	zipfiles.sort()
	test_dir = args.run_dir + '/generated_flattened_modules'
