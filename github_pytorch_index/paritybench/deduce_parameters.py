...
class DeduceParameters(object):
	...
	def __str__(self):
		return ', '.join(
			itertools.chain(
				map(str, self.args), [f'{name}={arg}' for name, arg in self.kwargs.items()]
			)
		)
	...
	def get_fixors(self):
		return [
			(r"missing.*arguments?: (?P<name>['][a-zA-Z0-9_]+['])", self.fix_missing_arg),
			(r"unexpected keyword argument (?P<name>['][a-zA-Z0-9_]+['])", self.fix_extra_arg),
			(r'size mismatch, m1: (?P<a>\[.*\]), m2: (?P<b>\[.*\])', self.fix_size_mismatch),
			(
				r"shape (?P<a>\[[\d, ]+\]) doesn't match the broadcast shape (?P<b>\[[\d, ]+\])]",
				self.fix_size_mismatch,
			),
			(r'assert +len\((?P<str_a>\w+)\) *== *len\((?P<str_b>\w+)\)', self.fix_equal_names),
...
class DeduceParameter(object):
	...
	def alt_guess(self, gen):
		if isinstance(self._guesses[-1], TensorGuess) and gen < 2:
			...
			new_size = [2, 3][gen]
			self.change_guess(
				TensorGuess([TensorGuess.default_size] * new_size, self._guesses[-1].dtype)
			)
	...
	def get_fix(self, error_message: str, line: str, pass_number: int, name: str):
		...
		if pass_number == 0 and isinstance(self.value, int):
			...
			fixors.extend([
				...
				(r'assert isinstance.*list', lambda: [TensorGuess.default_size] * 2),
				(
					r"AttributeError: 'int' object has no attribute '(size|shape|dtype|device|ndim)'",
					lambda: TensorGuess([TensorGuess.default_size] * 2),
				),
				(r'must be Tensor, not int', lambda: TensorGuess([TensorGuess.default_size] * 2)),
				...
				(r"AttributeError: 'int' object has no attribute 'split'", lambda: '2,2'),
				(
				...
				(r'should be a number in range \[0, 1\]', lambda: 0.5 if 'drop' in name else None),
				(r'member .* should be callable', lambda: _mock_layer()),
				(r"""assert.*in\s+[(\[{]\s*(?P<v>\d+|'[^'\\]*'|"[^"\\]*")""", lambda v: v),
				(
				...
				(
					...
					lambda: _mock_layer
					if any(s in name.lower() for s in ['norm', 'act', 'cls', 'block'])
					else None,
				),
				(
		...
		if pass_number == 1 and isinstance(self.value, int):
			fixors.extend([
				...
				(r'(NotImplementedError|AssertionError):[^\d]*\b(?P<val>\d+)\b', lambda val: val),
		...
		if pass_number == 0 and isinstance(self.value, list):
			...
			fixors.extend([
				...
				(r"unsupported operand .* 'list'", lambda: TensorGuess.default_size),
		...
		if pass_number == 0 and isinstance(self.value, torch.nn.Module):
			fixors.extend([
				(r"object has no attribute 'split'", lambda: type(self.value).__name__),
...
class TensorGuess(Guess):
	...
	def get_fix(self, error_message: str, line: str, pass_number: int, name: str):
		...
		other_fixors = [
			(r'expected Long', lambda: self.__class__([self.default_size], torch.int64)),
			(
			...
			(
				...
				lambda: (
					self.__class__([self.default_size], torch.int64) if 'len' in name else None
				),
			),
			(
			...
			(r'invalid combination of arguments.*Tensor', lambda: LiteralGuess(list(self.shape))),
			(
	...
	def shape_fixors(self, pass_number: int):
		if pass_number == 0:
			return [
				...
				(r'(?P<want>\d+) channels, but got (?P<got>\d+) channels', self.fix_num_channels),
				(
				...
				(r'matrices expected, got (?P<got>\d+)D, (?P<want>\d+)D ', self.fix_dimensions),
				(
					r'expected.* (?P<want>\d+)D input \(got (?P<got>\d+)D input\)',
					self.fix_dimensions,
				),
				(r'Expected.*size (?P<want>[\d, ()]+), got (?P<got>[\d, ()]+)', self.fix_shape),
				(
				...
				(r"Kernel size can't be greater than actual input size", self.fix_too_small),
		...
		if pass_number == 1:
			return [
				...
				(r'size mismatch, m1: (?P<a>\[.*\]), m2: (?P<b>\[.*\])', self.fix_size_mismatch2),
				(
					r"shape '(?P<view>\[[\d, -]+\])' is invalid for input of size (?P<size>\d+)",
					self.fix_view2,
				),
...
class ConfigGuess(Guess):
	...
	def get_fix(self, error_message: str, line: str, pass_number: int, name: str):
		guesses = sorted(self.value._guesses.values(), key=lambda x: x.created, reverse=True)
