...
# kernel path: /tmp/torchinductor_sahanp/x5/cx5tkz5d554qntnauidua5vc5he6pndim6xmavh3tchg5fo4gj3d.py
...
# kernel path: /tmp/torchinductor_sahanp/76/c76fzdtthv3e54gqs23rlho3tqbbbpnby5ggiv34qtajqm2yuk75.py
...
# kernel path: /tmp/torchinductor_sahanp/qq/cqq2xn3dbor7og55z4tikarigut6xxzf2sladuu2cidpyj3s2fyb.py
# Topologically Sorted Source Nodes: [x_1, x_2], Original ATen: [aten.replication_pad3d, aten.leaky_relu]
# Source node to ATen node mapping:
#   x_1 => _unsafe_index, _unsafe_index_1, _unsafe_index_2
#   x_2 => gt_24, mul_225, where
# Graph fragment:
...
#   %_unsafe_index : [num_users=1] = call_function[target=torch.ops.aten._unsafe_index.Tensor](args = (%slice_scatter_default_8, [None, None, %clamp_max, None, None]), kwargs = {})
...
#   %_unsafe_index_2 : [num_users=3] = call_function[target=torch.ops.aten._unsafe_index.Tensor](args = (%_unsafe_index_1, [None, None, None, None, %clamp_max_2]), kwargs = {})
#   %gt_24 : [num_users=1] = call_function[target=torch.ops.aten.gt.Scalar](args = (%_unsafe_index_2, 0), kwargs = {})
#   %mul_225 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%_unsafe_index_2, 0.01), kwargs = {})
#   %where : [num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%gt_24, %_unsafe_index_2, %mul_225), kwargs = {})
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
# kernel path: /tmp/torchinductor_sahanp/5g/c5gowxe35fs52w7dftbplsad6ouwrjvcfpjlx77mztavn22s2soo.py
# Topologically Sorted Source Nodes: [x_3, x_4, loss], Original ATen: [aten.relu, aten.mish, aten.mse_loss]
# Source node to ATen node mapping:
#   loss => mean, mul_236, pow_1
#   x_3 => relu
#   x_4 => exp, gt_25, log1p, tanh, where_1
# Graph fragment:
#   %relu : [num_users=4] = call_function[target=torch.ops.aten.relu.default](args = (%where,), kwargs = {})
#   %gt_25 : [num_users=1] = call_function[target=torch.ops.aten.gt.Scalar](args = (%relu, 20), kwargs = {})
#   %exp : [num_users=1] = call_function[target=torch.ops.aten.exp.default](args = (%relu,), kwargs = {})
#   %log1p : [num_users=1] = call_function[target=torch.ops.aten.log1p.default](args = (%exp,), kwargs = {})
#   %where_1 : [num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%gt_25, %relu, %log1p), kwargs = {})
#   %tanh : [num_users=1] = call_function[target=torch.ops.aten.tanh.default](args = (%where_1,), kwargs = {})
#   %mul_236 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%relu, %tanh), kwargs = {})
#   %pow_1 : [num_users=1] = call_function[target=torch.ops.aten.pow.Tensor_Scalar](args = (%mul_236, 2), kwargs = {})
...
# kernel path: /tmp/torchinductor_sahanp/kl/ckl4lqsnjh2hwazchptn2vky2tvd2ssbqn2fbrucfohpmicyldyv.py
# Topologically Sorted Source Nodes: [x_3, x_4, loss], Original ATen: [aten.relu, aten.mish, aten.mse_loss]
# Source node to ATen node mapping:
#   loss => mean, mul_236, pow_1
#   x_3 => relu
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        del buf1
        ps5 = 16 + 4*s2 + 4*s3 + s2*s3
        ps6 = 4 + s1
        ps7 = 4 + s3
        ps8 = 4 + s2
        ps9 = 64 + 16*s1 + 16*s2 + 16*s3 + 4*s1*s2 + 4*s1*s3 + 4*s2*s3 + s1*s2*s3
        ps10 = 16 + 4*s2 + 4*s3 + s2*s3
        ...
        # Topologically Sorted Source Nodes: [x_3, x_4, loss], Original ATen: [aten.relu, aten.mish, aten.mse_loss]
        triton_red_fused_mish_mse_loss_relu_3_r0_numel = (17 + 64*s0 + 16*s0*s1 + 16*s0*s2 + 16*s0*s3 + 4*s0*s1*s2 + 4*s0*s1*s3 + 4*s0*s2*s3 + s0*s1*s2*s3) // 18
        stream0 = get_raw_stream(0)
