...
def generate_prompt(num_modules: int = 10):
	...
	prompt = prompt_dict['generate_random_torch']['prompt'].replace(
		'{{modules}}', str(random_modules)
	)
	return prompt
...
def generate_random_torch_model(
	num_models: int = 1, model_name: str = 'deepseek-chat', max_workers: int = 50
):
	with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
		# Create a list of futures
		futures = [executor.submit(generate_single_model, model_name) for _ in range(num_models)]

		# Show progress bar for completed futures
		for _ in tqdm.tqdm(
			concurrent.futures.as_completed(futures), total=num_models, desc='Generating models'
		):
			pass
