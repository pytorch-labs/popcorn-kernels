...
def generate_patterns_pattern(
	operator_lists_with_ranges: list[tuple[list[str], tuple[int, int]]], p_value: float = 0.3
) -> list:
	"""
...
def check_if_model_exists_in_kernelbench(
	final_pattern: list[str], kernelbench_problem_set: set[str]
) -> bool:
	"""
	...
	if joined_pattern_name in kernelbench_problem_set:
		print(
			f'Model {joined_pattern_name} already exists in KernelBench Level 2!... we cannot use this since this will containmiate the data'
		)
		return True
	else:
		return False
...
def generate_synth_torch_single(
	...
	if config.model_name == 'local':
		response = generate_local_server_openai(
			prompt=prompt,
	...
	if os.path.exists(os.path.join(REPO_DIR, config.program_dir, f'{proposed_entry_point}.py')):
		...
		while os.path.exists(
			os.path.join(REPO_DIR, config.program_dir, f'{proposed_entry_point}_{suffix}.py')
		):
			suffix += 1
	...
	print(
		f'Successfully generate {file_name} with final pattern {final_pattern} | input pattern {pattern}'
	)
	if config.mode == 'parallel':
		num_current_files = num_generations_in_dir(os.path.join(REPO_DIR, config.program_dir))
		print(
			f'[Stats] {num_success.value} successful synthetic programs out of {num_generated.value} generated, yield {(num_success.value / num_generated.value) * 100:.2f}%, current total synthetic programs: {num_current_files}'
		)
...
def main(config: SynthConfig):
	...
	if config.mode == 'single':
		...
		print(
			f'Running in parallel mode, generating {config.num_total_samples} models with {config.num_worker} workers'
		)
